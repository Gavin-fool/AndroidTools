...
public class DeviceInformation {
	...
	public static String[] getRootDataPathItem(Context context) {
		...
		if (Build.VERSION.SDK_INT > 18) {
			pathItem = new String[] { "手机内存" + "  " + (list[0].split(","))[3] + "/" + (list[0].split(","))[2] };
			return pathItem;
		}
		// 遍历含有挂载点的字符串数组
		for (int i = 0; i < list.length; i++) {
			...
			if (Long.valueOf((list[i].split(","))[2]) != 0) {
				pathList.add("  " + (list[i].split(","))[3] + "/" + (list[i].split(","))[2]);
			}
		}
	...
	/** 获取当前挂载点的空间打下以及可用大小 */
	public static String getAvaiable(String s) {
		long blockSize;
		long blockCount;
		long availCount;
	...
	public static String getSIMCardInfo(Context context) {
		...
		return "{\"simOperator\":\"" + simOperator + "\",\"netWorkType\":\"" + netWorkType + "\",\"phoneType\":\"" + phoneType + "\",\"dataState\":\"" + dataState + "\",\"simState\":\"" + simState
				+ "\",\"subScriberId\":\"" + subScriberId + "\",\"networkCountryIso\":\"" + networkCountryIso + "\"}";
	}
	...
	public static String getDeviceBatteryInfo(Context context) {
		...
		return "{\"status\":\"" + status + "\",\"health\":\"" + health + "\",\"present\":\"" + present + "\",\"level\":\"" + level + "\",\"scale\":\"" + scale + "\",\"plugged\":\"" + plugged
	...
	public static String getDeviceSoftVersion() {
		return BaseApp.currentSoftVersion;
	}
	...
	/** 查找主目录路径 */
	public static String querryRootDataPath(Context context) {
		String[] list = DeviceInformation.getVolumePaths(context);
		File f = null;
		String path = "";
		for (int i = 0; i < list.length; i++) {
			path = list[i] + "/cgt_data_support/";
			f = new File(path);
			if (f.exists()) {
				return f.getPath();
			}
